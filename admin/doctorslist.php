<?php 
	...
	$pagenum = "";
	$per_page = 20;		
		$count = $db->query("SELECT * FROM doctordetails");		
		...
		$cat = $db->query("SELECT * FROM doctordetails LIMIT $start, $per_page");		
		if($cat->num_rows) {
			...
			while($row = $cat->fetch_assoc()) {				
				...
				$result .=  "<tr>
								...
								<td>$phno</td>
								<td>$hospital</td>
								<td><a href='doctorslist.php?delete=".$email."' onclick='return check();'><i class='pe-7s-close-circle'></i></a></td>
							</tr>";															
				$x++;
			}			
			$result .= "</tbody>
		...
		if($delete != "") {			
			$sql = $db->query("DELETE FROM doctordetails WHERE email='".$delete."'");		
			if($sql) {				
        ...
        <nav class="navbar navbar-default navbar-fixed" style="background: #000;">
            <div class="container-fluid">
                <div class="navbar-header">
                    ...
                    <a class="navbar-brand" href="#" style="color: #fff;">DOCTORS LIST</a>
                </div>
